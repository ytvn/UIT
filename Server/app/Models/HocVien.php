...
class hocvien extends BaseModel
{
    ...
    public static function checkMac($mahv, $MAC){
        ...
        //Kiểm tra xem có user nào dùng MAC này chưa, nếu chưa thì lưu mac vs time rồi cho pass
        $data = HocVien::select("MAC", "MACTIME", "MAHV")->where("MAC", $MAC)->first();
        //Trường hợp 2: User check in lần đầu, db chưa lưu mac, thì được pass luôn
        if($data==null)
        {
            HocVien::where("MAHV", $mahv)->update(['MAC' => $MAC , "MACTIME" => $now]);
            return '1';
        }
        //Trường hợp 3: Thiệt bị này đã được đăng nhập trên thiết bị khác
        $time = Carbon::createFromFormat("Y-m-d H:i:s", $data->MACTIME );
        $check = $now->diffInMinutes($time);
        if($check >10){//Nếu >15 kể từ lần điểm danh trc thì pass
            HocVien::where("MAHV", $data->MAHV)->update(['MAC' => null , "MACTIME" => null]);
            HocVien::where("MAHV", $mahv)->update(['MAC' => $MAC , "MACTIME" => $now]);
            return '1'; 
        }//Ngược lại thì tèo
        return 5;
