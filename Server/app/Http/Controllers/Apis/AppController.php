...
class AppController extends ApiController
{
    ...
    public function generateQR($MAMH){
        ...
        if(!$this->checkTime($MAMH))//Hàm trả true thì hợp lệ và cho phép tạo QR ngược lại reject
            return "Môn" . $MAMH . " không có tiết lúc này. Giảng viên kiểm tra lại thời khóa biểu rồi thử lại.";
    ...
    public function diemDanh($TOKEN, $MAHV, $MAC){
        ...
        // return HocVien::checkMac($MAHV, $MAC);
        if(HocVien::checkMac($MAHV, $MAC)=="5")
            return 5;// Thiết bị này đã dùng để đăng nhập tài khoản khác
        
        $string = MonHoc::whereRaw("TIMEDIFF('" . Carbon::now('Asia/Ho_Chi_Minh') . "',updated_at)  <10000 ")//now - updatee_at: đơn vị second
    ...
    public function checkTime($MAMH){
        ...
        $now =  Carbon::createFromFormat("H:i:s", $now->toTimeString());// NHỚ SỬA LẠI Ở ĐÂY NHA
